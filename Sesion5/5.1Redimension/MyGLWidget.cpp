...
void MyGLWidget::paintGL () //intercanvio pintar casa y suelo
{
  // Esborrem el frame-buffer
  glClear (GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT); //nuevo
  //tenemos que poner el glEnable(GL_DEPTH_TEST) para usar esto
  //Si no hay GL_DEPTH_BUFFER_BIT vemos el suelo por delante de la casa
  //Si hay GL_DEPTH_BUFFER_BIT se quita la parte del suelo que se sobrepone
/*
  // Activem el VAO per a pintar la caseta
  glBindVertexArray (VAO_Casa);
...
void MyGLWidget::modelTransform () 
{
  ...
  glm::mat4 transform = glm::scale(glm::mat4(1.0f), glm::vec3(scale));
  transform = glm::rotate(transform, rotHom, glm::vec3(0.,1.,0.));
  glUniformMatrix4fv(transLoc, 1, GL_FALSE, &transform[0][0]);
}
...
void MyGLWidget::projectTransform(){
    ...
    if(ra > 1){
    glm::mat4 Proj = glm::perspective(FOV, ra, 1.0, 3.0);
