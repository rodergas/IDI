...
void MyGLWidget::createBuffers ()
{
  ...
  first = true; // NUEVO v3
  // setMouseTracking(true); // NUEVO v4, por defecto estara a false
  // If mouse tracking is disabled, the widget only receives mouse move events 
...
void MyGLWidget::mouseMoveEvent (QMouseEvent *e) {
  ...
  if (first) { // NUEVO v3
	...
	vectorDeEscalado[0] += 0.05; // ++factor_escala_en_X
	modelTransform();
	updateGL();
      }
      else { // izquierda
	...
	vectorDeEscalado[1] -= 0.05; // --factor_escala_en_Y
	modelTransform();
	updateGL();
      }
      else { // arriba
